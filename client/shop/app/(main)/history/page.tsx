        ...
        return (
          <div key={step} className="flex items-center">
            <div className="flex flex-col items-center gap-1">
              ...
              <p
                className={`text-[9px] font-medium ${done ? "text-brand-primary-600" : "text-text-4"}`}
              >
                {meta.label}
              </p>
            </div>
            {i < TIMELINE_STEPS.length - 1 && (
              <div
                className={`mb-4 h-0.5 w-10 transition-all ${done && i < activeIdx ? "bg-brand-primary-600" : "bg-border-strong"}`}
              />
            )}
          </div>
        );
    ...
    >
      ...
      >
        ...
        <div className="text-right">
          <p className="font-bold text-brand-primary-600">
            {peso(Number(order.amount))}
          </p>
          <svg
            className={`ml-auto mt-1 text-text-4 transition-transform ${open ? "rotate-180" : ""}`}
            fill="none"
            height="14"
            stroke="currentColor"
            strokeWidth="2"
            ...
            <div className="space-y-3 border-t border-border-subtle px-4 pb-4 pt-3">
              ...
              <div className="rounded-xl bg-surface-bg-3 p-3 text-sm space-y-1.5">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-mono font-semibold text-brand-primary-600">
                    {order.personalCode}
                  </span>
                </div>
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-text-1">
                    {shortDateTime(order.holdEndsAt)}
                  </span>
                </div>
                {order.seller && (
              ...
              {order.events && order.events.length > 0 && (
                <div className="space-y-2">
                  <p className="text-[11px] font-semibold uppercase tracking-wide text-text-4">
                    Events
                  </p>
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-text-3">
          Track your purchases, sales, and timelines.
        </p>
      </div>
      ...
      >
        <ModalContent className="border border-border-subtle bg-surface-bg-2 text-text-1">
          ...
          <ModalBody className="space-y-4">
            {reviewOrder?.seller && (
              <div className="flex flex-col items-center gap-2">
                <Avatar
                  src={reviewOrder.seller.avatarUrl}
                  name={reviewOrder.seller.fullName}
                  size="lg"
                  isBordered
                  showFallback
                />
                <p className="font-semibold text-text-1">
                  {reviewOrder.seller.fullName}
                </p>
                <p className="text-xs text-text-3">
                  for: {reviewOrder.item?.title}
                </p>
              </div>
            )}
          ...
          <ModalFooter>
            <Button variant="light" onPress={() => setReviewOrder(null)}>
              Cancel
            </Button>
            <Button
              className="btn-cta"
