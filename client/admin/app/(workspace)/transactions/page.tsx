      ...
      <div className="flex items-center gap-0">
        {TIMELINE_STEPS.map((step, i) => (
          <div key={step} className="flex items-center">
            <div
              className={`flex h-7 w-7 items-center justify-center rounded-full text-[10px] font-bold transition-colors ${reached(step) ? "bg-brand-primary-600 text-white" : "bg-surface-bg-3 text-text-3 border border-border-subtle"}`}
            >
              {i + 1}
            </div>
            <span
              className={`ml-1.5 text-xs font-medium ${reached(step) ? "text-text-1" : "text-text-3"}`}
            >
              {step}
            </span>
            {i < TIMELINE_STEPS.length - 1 && (
              <div
                className={`mx-2 h-0.5 w-8 rounded ${reached(TIMELINE_STEPS[i + 1]) ? "bg-brand-primary-600" : "bg-border-subtle"}`}
              />
            )}
          </div>
        ))}
          ...
          {events.map((ev) => (
            <div key={ev.id} className="relative">
              ...
              <p className="text-[10px] text-text-3">
                {shortDateTime(ev.createdAt)} · {ev.actor}
              </p>
            </div>
          ))}
        </div>
...
export default function AdminTransactionsPage() {
  ...
  const handleForceComplete = async () => {
    ...
    try {
      ...
      setOrders((prev) =>
        prev.map((o) => (o.id === selected.id ? { ...o, ...res.order } : o)),
      );
      notifySuccess({
        title: "Order completed",
        description: `Order ${selected.id.slice(0, 10)}… force-completed.`,
      });
            ...
            >
              {(entry) => (
                <TableRow key={entry.id}>
                  ...
                  <TableCell>
                    {entry.buyer?.fullName ?? entry.buyerId.slice(0, 8)}
                  </TableCell>
                  <TableCell>
                    {entry.seller?.fullName ?? entry.sellerId.slice(0, 8)}
                  </TableCell>
                  <TableCell>
                    {entry.item?.title ?? entry.itemId.slice(0, 12)}
                  </TableCell>
              ...
              <div className="space-y-4 text-sm">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="rounded-lg border border-border-subtle bg-surface-bg-3 p-3">
                    <p className="mb-1.5 text-[10px] uppercase tracking-wide text-text-3">
                      Buyer
                    </p>
                    <div className="flex items-center gap-2">
                      <Avatar
                        name={selected.buyer?.fullName ?? "B"}
                        size="sm"
                        classNames={{
                          base: "bg-brand-primary-200 text-brand-primary-800 shrink-0",
                  ...
                  <div className="rounded-lg border border-border-subtle bg-surface-bg-3 p-3">
                    ...
                    <div className="flex items-center gap-2">
                      <Avatar
                        name={selected.seller?.fullName ?? "S"}
                        size="sm"
                        classNames={{
                          base: "bg-brand-cyan-200 text-brand-cyan-800 shrink-0",
                          name: "text-[10px] font-bold",
                        }}
                      />
                      <div className="min-w-0">
                ...
                <div className="rounded-lg border border-border-subtle bg-surface-bg-3 p-3 space-y-1.5">
                  ...
                  <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs text-text-2">
                    ...
                    <span>{shortDateTime(selected.createdAt)}</span>
                    {selected.personalCode && (
                      <>
                        <span className="text-text-3">Code</span>
                ...
                {selected.refund && (
                  <div className="rounded-lg border border-status-warning-300/40 bg-status-warning-50 p-3 text-xs space-y-1">
                    <p className="font-semibold text-status-warning-700">
                      Refund attached
                    </p>
                    <p className="text-text-2">
                      Status: {selected.refund.status}
                    </p>
                    <p className="text-text-2">
                      Policy: {selected.refund.policy}
                    </p>
                    <p className="text-text-2">
                      Buyer gets:{" "}
                      {peso(Number(selected.refund.buyerRefundAmount))}
                    </p>
                  </div>
                )}
              </div>
          ...
          <ModalBody>
            <p className="text-sm text-text-2">
              ...
              <span className="font-mono text-text-1">
                {selected?.id.slice(0, 14)}…
              </span>{" "}
              as COMPLETED and trigger seller payout. This action cannot be
              undone.
            </p>
            {selected && (
