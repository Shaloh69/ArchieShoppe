...
function KioskSellSlotContent() {
  ...
  const onContinue = () => {
    ...
    router.push(`/kiosk/sell/place?slot=${slot}&item=${encodeURIComponent(item)}`);
  };
    ...
    <div className="mx-auto flex min-h-[calc(100vh-3rem)] w-full max-w-3xl items-center">
      <Card className="w-full border border-border-subtle bg-surface-bg-2">
        <CardBody className="space-y-5 p-8 text-center">
          <h1 className="text-3xl font-semibold text-text-1">Your slot is #{slot}</h1>
          <p className="text-lg text-text-2">Open the slot and place your item.</p>
...
export default function KioskSellSlotPage() {
  return (
    <Suspense fallback={<div className="text-text-2">Loading slot details...</div>}>
